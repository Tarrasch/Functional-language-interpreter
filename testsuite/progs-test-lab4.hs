...
listGoodProgs = listFunFiles "good"

listBadProgs = listFunFiles "bad"

listFunFiles dir = 
    liftM (map (\f -> joinPath [dir,f]) . sort . filter ((=="fun") . getExt)) $ getDirectoryContents dir
...
runMakeClean :: FilePath -> IO ()
runMakeClean dir = do checkDirectoryExists dir
                      runCommandNoFail_ ("make clean " ++ quote dir) ""
